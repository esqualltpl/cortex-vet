...
namespace App\Http\Controllers\Student\NeuroAssessment;
...
class NeuroAssessmentController extends Controller
{
    ...
    {
        return view('student.neuroAssessment.index');
    }
    ...
    public function patientIdInfo(Request $request)
    {
        ...
        try {
            if ($patientInfo = Patient::where('patient_id', $request->patient_id)->where('added_by', auth()->user()->id)->first()) {
                ...
                return redirect()->route('student.neuro.assessment.exam', Crypt::encrypt($patientInfo->id))->with($response);
            }
...
//            $response['redirect_url'] = route('student.neuro.assessment.exam', $id);
            $response['redirect_url'] = route('student.neuro.assessment');
